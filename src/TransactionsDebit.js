...
export class TransactionsDebit extends Component {
  constructor(props) {
    super();
  ...
  getTransactions(){
    fetch(`${SERVER_URL}/mobile/get_rider_debit_transactions/${this.state.user.id}`, {
      method: 'GET'
   })
  ...
  filter(){
    ...
    fetch(`${SERVER_URL}/mobile/get_rider_debit_transactions_filter/${this.state.user.id}/${this.state.from}/${this.state.to}`, {
      method: 'GET'
   })
   .then((response) => response.json())
   .then((res) => {
      ...
      <View style = {styles.body}>
        ...
        <View style={styles.linkRow}>
          <TouchableOpacity onPress={() => this.props.navigation.push('CompanyTransactions')} style={styles.linkCol1}>
            <Text style={styles.linkText1}>Credit</Text>
          </TouchableOpacity>
          <TouchableOpacity  style={styles.linkCol2}>
            <Text style={styles.linkText2}>Debit</Text>
          </TouchableOpacity>
        </View>
          ...
          {this.state.transactions && this.state.transactions.map((transaction, index) => (
            <TouchableWithoutFeedback key={index} onPress={() => {/*this.gotoProductDetailsPage(review)*/} }>
              <View style={styles.itemView}>
                <View style={styles.item1}>
                  <View style={styles.contentCol3}>
                    <Text style = {styles.pName}>TRN REF: {transaction.trn_ref} </Text>
