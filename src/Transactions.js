...
export class Transactions extends Component {
  ...
  getTransactions(){
    this.showLoader();
    fetch(`${SERVER_URL}/mobile/get_rider_transactions/${this.state.user.id}`, {
      method: 'GET'
   })
  ...
  filter(){
    ...
    this.showLoader();
    fetch(`${SERVER_URL}/mobile/get_rider_transactions_filter/${this.state.user.id}/${this.state.from}/${this.state.to}`, {
      method: 'GET'
   })
   .then((response) => response.json())
   .then((res) => {
...
displayCashout(){
  if(this.state.user.vendor_id == null){
    return(
      <TouchableOpacity style = {styles.editText} onPress={() => this.props.navigation.navigate('RiderBalance')}>
            <Text style = {styles.editT}>Cashout</Text>
          </TouchableOpacity>
    )
  }
}
...
displayNoData(){
  var data = this.state.transactions.length;
  if(data < 1){
    return(
    ...
    return (
      <View style = {styles.body}>
        ...
        <Icon name="arrow-back" size={18} color="#fff"  style = {styles.menuImage}/>
          ...
          <Text style = {styles.headerText}>Your commissions</Text>
        ...
        <ScrollView style={styles.sView} showsVerticalScrollIndicator={false}>
                ...
                <View style={styles.item1}>
                  <View style={styles.contentCol3}>
                    ...
                    <View style={{flexDirection: 'row', paddingTop: 5}}>
                      ...
                      <Text style = {styles.labela}>Commission: ₦{parseFloat(transaction.commission_amount).toFixed(2).replace(/\d(?=(\d{3})+\.)/g, '$&,')} </Text>
                    </View>
                  </View>
                </View>
              </View>
          ...
          {this.state.transactions && this.displayNoData()}
