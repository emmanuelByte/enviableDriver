...
export class CompanyRiderTransactions extends Component {
  constructor(props) {
    super();
  ...
  getTransactions(){
    this.showLoader();
    fetch(`${SERVER_URL}/mobile/get_rider_transactions/${this.state.rider.id}`, {
      method: 'GET'
   })
  ...
  filter(){
    ...
    this.showLoader();
    fetch(`${SERVER_URL}/mobile/get_rider_transactions_filter/${this.state.rider.id}/${this.state.from}/${this.state.to}`, {
      method: 'GET'
   })
   .then((response) => response.json())
   .then((res) => {
    ...
    return (
      <View style = {styles.body}>
        ...
        <Icon name="arrow-back" size={18} color="#fff"  style = {styles.menuImage}/>
          ...
          <Text style = {styles.headerText}>{this.state.rider.first_name}'s commissions</Text>
        </LinearGradient>
...
export default CompanyRiderTransactions
