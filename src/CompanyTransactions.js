...
export class CompanyTransactions extends Component {
  constructor(props) {
    super();
  ...
  getTransactions(){
    this.showLoader();
    fetch(`${SERVER_URL}/mobile/get_vendor_transactions/${this.state.user.vendor_id}`, {
      method: 'GET'
   })
   ...
   .then((res) => {
       ...
       if(res.success){
          this.setState({
            ...
            totalTransaction: res.transactions.reduce(function (a, b) { return parseFloat(a) + parseFloat(b.commission_amount); }, 0)
          });
       }else{
  ...
  filter(){
    ...
    this.showLoader();
    fetch(`${SERVER_URL}/mobile/get_vendor_transactions_filter/${this.state.user.vendor_id}/${this.state.from}/${this.state.to}`, {
      method: 'GET'
   })
   .then((response) => response.json())
   .then((res) => {
...
this.setState({
  ...
  setToDate = (event, date) => { this.setState({to: moment(date).format('MM/DD/YYYY'), toInput: false})}; 
    ...
    return (
      <View style = {styles.body}>
        ...
        <View style={styles.linkRow}>
          <TouchableOpacity  style={styles.linkCol1}>
            <Text style={styles.linkText1}>Earnings</Text>
          </TouchableOpacity>
          <TouchableOpacity onPress={() => this.props.navigation.push('CompanyTransactionsDebit')} style={styles.linkCol2}>
        ...
        <View style={styles.itemView}>
          <View style={styles.item1}>
            <View style={styles.contentCol3}>
              <Text style = {styles.pName5}>Total: ₦{parseFloat(this.state.totalTransaction).toFixed(2).replace(/\d(?=(\d{3})+\.)/g, '$&,')} </Text>
            </View>
          </View>
        </View>
          ...
          {this.state.transactions && this.state.transactions.map((transaction, index) => (
            <TouchableWithoutFeedback key={index} onPress={() => {/*this.gotoProductDetailsPage(review)*/} }>
              <View style={styles.itemView}>
                <View style={styles.item1}>
                  <View style={styles.contentCol3}>
                    <Text style = {styles.pName}>Order no: {transaction.order_number} </Text>
                    <Text style = {styles.desc}>Delivery fee: ₦{parseFloat(transaction.delivery_fee).toFixed(2).replace(/\d(?=(\d{3})+\.)/g, '$&,')} </Text>
                    <Text style = {styles.desc}>Driver: {transaction.rider_first_name} {transaction.rider_last_name}</Text>
                    <View style={{flexDirection: 'row', paddingTop: 5}}>
                      <Text style = {styles.date}><TimeAgo time={transaction.created_at} /></Text>
                      <Text style = {styles.label}>Commission: ₦{parseFloat(transaction.commission_amount).toFixed(2).replace(/\d(?=(\d{3})+\.)/g, '$&,')} </Text>
                    </View>
                  </View>
                </View>
              </View>
