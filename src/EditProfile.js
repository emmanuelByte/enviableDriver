...
export class EditProfile extends Component {
  ...
  prepareImage(){
    ...
    data.append("riderId", this.state.user.id);
    this.register(data);
  }
        ...
        <ScrollView keyboardShouldPersistTaps={'always'} showsVerticalScrollIndicator={false} >
          ...
          <Text style = {styles.headerText}>Update profile</Text>
            ...
            <TouchableOpacity  onPress={() => this.openImagePicker()}>
              {this.state.user.photo && !this.state.imageUri && 
                <Image source = {{uri: `${SERVER_URL+ '/'+this.state.user.photo}`}} style = {styles.imgBg} />
              }
              {this.state.imageUri && 
                <Image source = {{uri: this.state.imageUri}} style = {styles.imgBg1} />
              }
            <Text style = {styles.logoText}>Change profile photo</Text>
            </TouchableOpacity>
