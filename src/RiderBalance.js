...
export class RiderBalance extends Component {
  constructor(props) {
    super();
  ...
  getTransactions(){
    this.showLoader();
    fetch(`${SERVER_URL}/mobile/get_rider_debit_transactions1/${this.state.user.id}`, {
      method: 'GET'
   })
    ...
    return (
      <View style = {styles.body}>
        ...
        <Icon name="arrow-back" size={18} color="#fff"  style = {styles.menuImage}/>
          ...
          <Text style = {styles.headerText}>Cashout history</Text>
        ...
        <ScrollView style={styles.sView} showsVerticalScrollIndicator={false}>
          ...
          {this.state.transactions && this.state.transactions.map((transaction, index) => (
            <TouchableWithoutFeedback key={index} onPress={() => {/*this.gotoProductDetailsPage(review)*/} }>
              <View style={styles.itemView}>
                <View style={styles.item1}>
                  <View style={styles.contentCol3}>
                    <Text style = {styles.pName}>{transaction.status} </Text>
         ...
         <View style={styles.itemView}>
          <View style={styles.item1}>
            <View style={styles.contentCol3}>
              <Text style = {styles.pName5}>Total withdrawal: ₦{parseFloat(this.state.totalTransactions).toFixed(2).replace(/\d(?=(\d{3})+\.)/g, '$&,')} </Text>
            </View>
          </View>
        </View>
        </ScrollView>
